  ...
  const refreshLocation = async () => {
    ...
    try {
      ...
      if (permission.status !== Location.PermissionStatus.GRANTED) {
        setCoordinates(null);
        setErrorMessage('Permita a localização para validar a geofence do dispositivo.');
        return;
      }
      ...
      setCoordinates(null);
      setErrorMessage(error instanceof Error ? error.message : 'Não foi possível obter a localização do tablet.');
    } finally {
      setIsLoading(false);
    }
  };
